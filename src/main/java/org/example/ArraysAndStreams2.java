...
public class ArraysAndStreams2 {
   public static void main(String[] args) {
      ...
      System.out.printf("strings starting with vowel: %s%n",
              Arrays.stream(strings)
                      .map(String::toLowerCase)
                      .filter(s -> s.startsWith("a") || s.startsWith("e") || s.startsWith("i") || s.startsWith("o") || s.startsWith("u"))
                      .collect(Collectors.toList()));

      System.out.printf("strings concatenated together: %s",
              Arrays.stream(strings)
                      .collect(Collectors.joining(""))
      );

      System.out.println("\nStrings of 5 or more characters: " + Arrays.stream(strings).filter(s -> s.length() >= 5).count());
